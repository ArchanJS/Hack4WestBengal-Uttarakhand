...
let UserRegister = () => {
    ...
    useEffect(()=>{
        if(localStorage.getItem("react-social-token")) history.push('/');
    })
  ...
  const handleAddrTypeChange = (e) => { 
      setUser({...user, businessType:addrtype[e.target.value]})
  }
    ...
    let submitRegistration = (event) => {
        ...
        if(user.name.trim() !== '' && user.email.trim() !== '' && user.password.trim() !== '' && user.businessType.trim() !== 'Select_your_business_type'){
